// Source: https://github.com/WindowsNT/ScreenCapture
// With modifications
...
inline void EnumVistaMixers(std::vector<VISTAMIXER>& vistamixers)
{
	...
	if (deviceEnumerator)
	{
		...
		deviceEnumerator->EnumAudioEndpoints(eCapture, DEVICE_STATE_ACTIVE, &e);
		...
		deviceEnumerator->EnumAudioEndpoints(eRender, DEVICE_STATE_ACTIVE, &e);
...
class CAPTURE
{
	public:
		...
		HRESULT CreateDirect3DDevice(IDXGIAdapter1* g)
		{
			...
			{
				hr = D3D11CreateDevice(g, DriverTypes[DriverTypeIndex], nullptr, D3D11_CREATE_DEVICE_VIDEO_SUPPORT, FeatureLevels, NumFeatureLevels, D3D11_SDK_VERSION, &device, &FeatureLevel, &context);
		...
		bool Get(IDXGIResource* lDesktopResource, bool Curs, CAPTURE cap, RECT* rcx = 0)
		{
			...
			if (nullptr == lAcquiredDesktopImage)
			{
				cap.lDeskDupl->ReleaseFrame();
				return 0;
			}*/
			...
			// QI for IDXGISurface	
			//hr = lAcquiredDesktopImage->QueryInterface(__uuidof(IDXGISurface), (void**)&ipCopySurface);
			...
			auto sz = lOutputDuplDesc.ModeDesc.Width * lOutputDuplDesc.ModeDesc.Height * multi;
			//auto sz2 = sz;
			buf.resize(sz);

			memcpy(buf.data(), resource.pData, sz);
		...
		bool Prepare(UINT Output = 0)
		{
			...
			hr = lDxgiOutput1->DuplicateOutput(device, &lDeskDupl);
			...
			if (desc.Format == DXGI_FORMAT_R16G16B16A16_FLOAT) // HDR, not GDI compatible
			{
				InHDR = desc.Format;
			}
			else
			...
			if (desc.Format == DXGI_FORMAT_R16G16B16A16_FLOAT) // HDR
			{
				InHDR = desc.Format;
				IsHDR = true;
			}
			...
			Sleep(50); // First frame blank fix
		...
		HRESULT Convert(void* f, int wi, int he, std::vector<DWORD>& out, GUID from = GUID_WICPixelFormat128bppPRGBAFloat)
		{
			if (!wbfact)
			{
				CoCreateInstance(CLSID_WICImagingFactory2, 0, CLSCTX_INPROC_SERVER, __uuidof(IWICImagingFactory2), (void**)&wbfact);
			}
...
struct DESKTOPCAPTUREPARAMS
{
	...
	std::function<HRESULT(const BYTE* d, size_t sz, UINT wi, UINT he, void* cb)> Framer;
...
inline int DesktopCapture(DESKTOPCAPTUREPARAMS& dp)
{
	...
	if (dp.HasAudio)
	{
		...
		LPWSTR default_device_id = 0;
		pDevice->GetId(&default_device_id);

		if (default_device_id)
		{
			//vm.id = default_device_id;
		...
		/*CComPtr <IPropertyStore> st = 0;
		pDevice->OpenPropertyStore(STGM_READ, &st);
		...
		dp.AudioFrom = {
			//vistamixers[0].id, vistamixers[1].id,
			//vistamixers[1].id,
			//{ vistamixers[0].id, {0,1} }
			{ default_device_id, {0,1} }
		};
	}
	...
	if (dp.HasVideo && !dp.Framer)
	{
		...
		if (
			...
			dp.VIDEO_ENCODING_FORMAT == MFVideoFormat_HEVC
			||
			dp.VIDEO_ENCODING_FORMAT == MFVideoFormat_HEVC_ES
			||
			dp.VIDEO_ENCODING_FORMAT == MFVideoFormat_VP80
			||
			dp.VIDEO_ENCODING_FORMAT == MFVideoFormat_VP90
			)
		{
		...
		HRESULT hr = pMediaTypeOutVideo->GetUINT32(MF_MT_DEFAULT_STRIDE, (UINT32*)&lStride);
		...
		if (FAILED(hr))
		{
			...
			(void)pMediaTypeOutVideo->SetUINT32(MF_MT_DEFAULT_STRIDE, UINT32(lStride));
		}
	...
	hr = pAudioClient->Initialize(AUDCLNT_SHAREMODE_SHARED, flg, rt, 0, pwfx, NULL);*/
	...
	for (;;)
	{
		...
		if (dp.HasVideo)
		{
			...
			/*if (!lFrameInfo.LastPresentTime.QuadPart)
			{
				continue;
			}*/
			...
			if (lDesktopResource && !cap.Get(lDesktopResource, dp.Cursor, cap, dp.rx.right && dp.rx.bottom ? &dp.rx : 0))
			{
				break;
			}
			...
			else
			{
				...
				// Calculate the DT (delta time) with last frame and set to the frame property
			...
			else if (cap.InHDR == DXGI_FORMAT_R16G16B16A16_FLOAT)
			{
			...
			else
			{
				if (!dp.Framer)
				{
					// Vertically flipped fix, source: https://github.com/opencv/opencv/pull/5661
					LONG stride = wi * multi;
					hr = MFCopyImage(
						pData,								// Destination buffer
						stride,								// Destination stride
						cap.buf.data() + (he - 1) * stride,	// First row in source image
				...
				else
				{
					...
					/*size_t sz = std::min(cap.buf.size(), (size_t)VideoBufferSize);
					memcpy(pData, cap.buf.data(), sz);*/
				}
			}

			if (dp.Framer)
			{
				hrf = dp.Framer(pData, std::min(cap.buf.size(), (size_t)VideoBufferSize), wi, he, dp.cb);
			}
